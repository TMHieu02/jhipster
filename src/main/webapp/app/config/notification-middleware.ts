...
export default () => next => action => {
  ...
  if (isRejectedAction(action) && error && error.isAxiosError) {
    ...
    } else if (error.message && (error.message.includes('proxy') || error.message.includes('ECONNREFUSED') || error.message.includes('Network Error'))) {
      // Handle proxy/network errors
      addErrorAlert('Server not reachable', 'error.server.not.reachable');
    } else {
      // For other errors, try to translate if possible, otherwise show the message
